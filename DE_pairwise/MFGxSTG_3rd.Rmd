...
work_plots = "~/ad-omics/ricardo/tmp/pairwise_dream/"
...
samples_reg1 = as.character(metadata$donor_tissue[metadata$tissue %in% "MFG"])
samples_reg2 = as.character(metadata$donor_tissue[metadata$tissue %in% "STG"])
rownames(metadata) <- metadata$donor_tissue
...
res_dream <- data.frame(topTable(fitmm, coef='tissueSTG', 
...
write.table(res_name, file = paste0(work_plots, "MFGxSTG_dream_3rd.txt"), quote = F, sep = "\t")

```

### Directionality 
LogFC > 0 is up in STG from **Dream**. 
