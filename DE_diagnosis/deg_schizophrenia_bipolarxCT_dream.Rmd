...
subtitle: "Schizophrenia + Bipolar x CT"
author:  |
...
The Bipolar and Schizophrenia samples are under the main_diagnosis = **Psychiatric Diagnosis**. 
```{r pick.samples, echo=TRUE}
metadata <- metadata3rd_pass

samples_case = as.character(metadata$donor_tissue[metadata$diagnosis %in% "Schizophrenia" | metadata$diagnosis %in% "Bipolar disorder"])
...
write.table(res_name, file = paste0(work_plots, "scz_bipolarxct_dream.txt"), quote = F, sep = "\t")
